...
class PatternDesign():
    ...
    def drawArmscye(self, path_svg, points, distance_back, distance_front):
        return self.drawArmscyeBack(path_svg, points[0:5], distance_back) + self.drawArmscyeBack(path_svg, points[4:], distance_front) 

    def drawArmscyeBack(self, path_svg, points, distance_back):
        length = 0
        back_neck, back_shoulder, mid_armscye_back, bottom_armscye_back, bottom_armscye = points
    ...
    def drawArmscyeFront(self, path_svg, points, distance_front):
        length = 0
        bottom_armscye, bottom_armscye_front, mid_armscye_front, front_shoulder, front_neck = points
    ...
    def drawBodiceBack(self, path_svg, pts):
        ...
        self.drawArmscyeBack(path_svg, [pts[9], pts[11], pts[14], pts[12], pts[24]], distance_12)
    ...
    def drawBodiceFront(self, path_svg, pts):
        ...
        self.drawArmscyeFront(path_svg, [pts[24], pts[18], pts[19], pts[23], pts[17]], distance_18)
        return

    def moveAllPoints(self, pts, offset_x, offset_y):
    ...
    def makeBlock(self, base_pt):
        ...
        self.drawBodiceBack(path_svg, pts)
        self.moveAllPoints(pts, 1 * IN_TO_PX, 0)
        self.drawBodiceFront(path_svg, pts)
        # end of pattern piece

        # Sample dart manipulation
        # TODO: Replace with an SVG transform if we can figure out how to add groups
        self.rotateDart([pts[17], pts[20], pts[15]], [pts[24], pts[19], pts[23], pts[18], pts[27]])
        self.moveAllPoints(pts, lineLengthP(pts[26], pts[8]) + 1 * IN_TO_PX, 0)
        moveP(path_svg, pts[24])
        lineP(path_svg, pts[27])
        lineP(path_svg, pts[20])
        lineP(path_svg, pts[28])
        lineP(path_svg, pts[26])
        lineP(path_svg, pts[8])
        lineP(path_svg, pts[16])
    ...
    def pattern(self):
        ...
        metainfo = {'companyName':'Test Company',      # mandatory
                    ...
                    'patternName':'Sample dart manipulation',  # mandatory
